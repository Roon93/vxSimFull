...
#include "config.h" /*BSP的头文件*/
	...
	.long	FUNC(copyright_wind_river) /*定义一个32-bit的全局变量，变量的初始值为_copyright_wind_river的地址。由于在Makefile中规定了romInit.o为第一个链接的模块，所以这个无名变量将出现在数据段的最开始（没有变量名）*/
...
romWarmLow:				/* ROM_WARM_LOW(0x20) is offset */
	...
	.balign 16,0x90
cold:
	.byte	0x67, 0x66		/* next inst has 32bit operand 0X66用于反转默认的操作数大小! 0X67用于反转默认的寻址方式*/
