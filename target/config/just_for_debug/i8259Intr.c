...
*------------------------------------------------------------------
*|/ | / | / | 1  | LTIM     | ADI         | SNGL     | IC4        |
*|  |   |   |标志|1:电平触发|1:调用间隔为4|1:单片方式|1:写入ICW4  |
*|  |   |   |位  |0:边沿触发|0:调用间隔为8|0:级联方式|0:不写入ICW4|
*------------------------------------------------------------------
*
*ICW2:
* D7  D6  D5  D4  D3  D2    D1    D0
...
* D7  D6  D5  D4                D3       D2      D1           D0
*------------------------------------------------------------------------
*|0 | 0 | 0 |SFNM            | BUF    | M/S  | AEOI       | uP          |
*|  |   |   |1:特殊完全嵌套  |1:缓冲  |1:主片|1:自动结束  |1:80X86模式  |
*|  |   |   |0:非特殊完全嵌套|0:非缓冲|0:从片|0:非自动结束|0:非80X86模式|
*------------------------------------------------------------------------
*/
VOID i8259Init (void)
{
	...
	/*PIC_port1 == (base+(0x00*1))*/
	/*PIC_port2 == (base+(0x01*1))*/
	/*往0x20和0x21端口分别写入数据*/
...
/*I8259_SPECIAL_FULLY_NESTED_MODE特殊完全嵌套*/
