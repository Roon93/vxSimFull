/* trcLib.c - MC680X0 stack trace library */
...
--------------------
02a,18jan02,aeg  added 1 byte to name buffer passed to symFindByValue().
01z,12may95,p_m  adapted to support host based tt().
01y,15oct92,kdl  made trcCountArgs recognized multiple ADDQ's at return addr
		 (SPR #1650).
01x,24sep92,yao  made document consistent with change 01u.
01w,14sep92,yao  made trcStack NOMANUAL.
01v,26may92,rrr  the tree shuffle
01u,30mar92,yao  added third paramatet tid to trcStack().  changed copyright
		 notice.  shut up ANSI warnings.
01t,04oct91,rrr  passed through the ansification filter
                  -changed functions to ansi style
		  -changed VOID to void
		  -changed copyright notice
01s,20may91,jdi  documentation tweaks.
01r,30mar91,jdi  documentation cleanup; doc review by dnw.
01q,10jun90,dnw  changed to print default number of args (5) if unable to
		   determine actual number from call.  default args shown as
		   "([..])".
		 improved ability to trace without LINK by using symbol tbl
01p,10aug90,dnw  added forward declarations for void functions.
01o,15jcf90,jcf  changed symbol type to SYM_TYPE.
01n,09aug89,gae  fixed documentation example for taskRegsGet.
01m,30may88,dnw  changed to v4 names.
01l,27jan88,jcf  documentation.
01k,04nov87,ecs  documentation.
01j,01apr87,ecs  minor documentation about vxTaskRegs in trcStack.
01i,24mar87,jlf  documentation
01h,19mar87,ecs  fixed bug in trcStack introduced in 01g. Did this by moving
		    test for NULL fp from trcStack to trcStackLvl.
		 removed unnecessary sp parameter from trcStackLvl.
01g,25feb87,ecs  changed trcFollowBra to not follow branches to odd addresses,
		    nor to follow (immediately) circular branches.
		 changed trcCountArgs to say "8" when confronted with
		    MOVE.L (xxx,A6),A7.
		 added includes of symLib.h and sysSymTbl.h.
		 changed trcFindFuncStart to use symbol table as a lower bound.
01f,21dec86,dnw  changed handling of NULL print routine to appease Green Hills.
		 changed to not get include files from default directories.
01e,05dec86,ecs  fixed trcCountArgs to handle MOVE.L xxx,(A7).
01d,18nov86,llk  fixed trcCountArgs.  Gets the number of args for ADDQ.W.
01c,27jul86,llk  prints error messages to standard error (uses printErr)
01b,02jul86,dnw  deleted obsolete trcTask() and trcTaskXP() routines.
		 documentation.
01a,03mar86,dnw  extracted from dbgLib.c
		 changed to handle routines that start w/BRA to LINK.
		 fixed bug of not handling negative displacements in BRAs.
		 changed to handle 68020 32-bit displacement form of BRAs.
*/
