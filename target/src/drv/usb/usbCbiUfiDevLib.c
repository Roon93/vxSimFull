...
SEE ALSO:
.I "USB Mass Storage Class - Control/Bulk/Interrupt Transport Specification Revision 1.0,"
.I "USB Mass Storage Class - UFI Command specification Revision 1.0"
...
LOCAL SEM_HANDLE   cbiUfiIrpSem;      /* Semaphore for IRP Synchronisation */
...
LOCAL VOID usbCbiUfiAttachCallback
    ...
    UINT16 deviceSubClass,      /* Device sub-class  - 0x4  for UFI command  */    UINT16 deviceProtocol       /* Interfaceprotocol - 0x00 for CBI          */
    )
    {
    ...
    switch (attachAction)
        ...
        case USBD_DYNA_ATTACH: 
            ...
            if ((pCbiUfiDev = usbCbiUfiPhysDevCreate (nodeId, 
						      configuration, 
                                                      interface)) ==NULL )
                {
...
*
* ERRNO:
*   S_usbCbiUfiDevLib_OUT_OF_RESOURCES
*   S_usbCbiUfiDevLib_USBD_FAULT
*/
...
STATUS usbCbiUfiDevInit (VOID)
    ...
    if (initCount == 0)
        ...
        if (OSS_SEM_CREATE (1, 1 , &cbiUfiIrpSem) != OK)
            return (usbCbiUfiDevShutDown (S_usbCbiUfiDevLib_OUT_OF_RESOURCES));
...
STATUS usbCbiUfiDevShutDown 
    ...
    if (cbiUfiIrpSem != NULL)
        {
        OSS_MUTEX_DESTROY (cbiUfiIrpSem);
        cbiUfiIrpSem = NULL;
        }
...
LOCAL VOID usbCbiUfiDevDestroy
    ...
    if (pCbiUfiDev != NULL)
        ...
        OSS_SEM_TAKE (cbiUfiIrpSem, OSS_BLOCK); 
...
LOCAL pUSB_CBI_UFI_DEV usbCbiUfiPhysDevCreate
    ...
    pCbiUfiDev->configuration   = configuration;
...
BLK_DEV * usbCbiUfiBlkDevCreate 
    ...
    /* 
     * Initialise the standard block device structure for use 
     * with file systems 
     */
...
LOCAL STATUS usbCbiUfiDevStChk
    ...
    /* Form a Test Unit Ready command and send it */ 
 
    if ( usbCbiUfiFormCmd (pCbiUfiDev, USB_UFI_TEST_UNIT_READY, 0, 0) != OK )
        {
    ...
    if (usbCbiUfiCmdExecute (pCbiUfiDev) != USB_COMMAND_SUCCESS)
        ...
        USB_CBI_UFI_DEBUG ("usbCbiUfiDevStChk: device not ready\n",
                           0, 0, 0, 0, 0, 0);
        OSS_FREE (pSenseData);
    ...
    else if (pCbiUfiDev->intrStatus[0] != USB_UFI_NO_SENSE) 
        {
        ...
        else if (pCbiUfiDev->intrStatus[0] == USB_UFI_ASC_MEDIA_CHANGE) 
            {
            USB_CBI_UFI_DEBUG ("usbCbiUfiDevStChk: Media changed\n",
                               0, 0, 0, 0, 0, 0);
	    ...
	    /*  
	     *  Re-submit the read capacity command now that a disk
	     *	has been inserted
	     */
        ...
        /* For all other sense data return ERROR */ 
...
*
* ERRNO:
*   S_usbCbiUfiDevLib_BAD_PARAM
*   S_usbCbiUfiDevLib_OUT_OF_MEMORY
*/
...
*
* ERRNO:
*   S_usbCbiUfiDevLib_NOT_REGISTERED
*/
